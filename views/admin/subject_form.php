...
<div class="section-header">
    <h2>
        ...
        <?= $isEdit ? 'Редагувати' : 'Створити' ?> предмет
    </h2>
</div>

<form method="POST" action="/admin/subjects/<?= $isEdit ? 'edit/' . $subject['id'] : 'create' ?>">
    <input type="hidden" name="csrf_token" value="<?= $csrfToken ?>">
    
    <div class="mb-3">
        <label for="name" class="form-label">Назва предмета *</label>
        <input type="text" class="form-control" id="name" name="name" 
               value="<?= $isEdit ? $this->e($subject['name']) : '' ?>" 
               required maxlength="100">
    </div>
    
    <div class="mb-3">
        <label for="description" class="form-label">Опис</label>
        <textarea class="form-control" id="description" name="description" rows="3"><?= $isEdit ? $this->e($subject['description'] ?? '') : '' ?></textarea>
    </div>
